...
const StatusUpdates = () => {
  ...
  useEffect(() => {
    ...
    if (Array.isArray(calendarData) && formattedSelectedDate) {
      const foundStatus = calendarData.find(
      ...
      <form className="form">
        ...
        <div className="row">
          <div className="col-12">
            <StatusCalendar
              data={formattedData}
              empName={auth.currentUser?.displayName || ''}
            />
          </div>
        </div>
        {msgResponse && (
