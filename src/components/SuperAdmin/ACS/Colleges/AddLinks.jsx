...
export const AddLinks = () => {
  ...
  const handleSaveEdit = () => {
    ...
    if (collegeIdToUpdate) {
      ...
      fetch(
        'http://127.0.0.1:8000/college_details/${collegeIdToUpdate}/update/',
        requestData
      )
        .then((res) => {
          if (!res.ok) throw new Error('Network response was not ok');
      ...
      fetch('http://127.0.0.1:8000/college_details/create/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
                ...
                {editIndex === index ? (
                  <>
                    <InputField
                      ...
                      setError={(error) =>
                        handleFieldError(`college_label_${index}`, error)
                      }
                    />
                    <InputField
                      disabled={inputDisabled}
                      ...
                      setError={(error) =>
                        handleFieldError(`link_${index}`, error)
                      }
                    />
                    <i
                      className="icon_color bi bi-check pt-4 icon-400"
                  ...
                  <>
                    ...
                    />
                    <i
                      className="icon_color_edit bi bi-pencil ml-2 pt-4"
                      onClick={() => handleEditClick(index)}
                      style={{ cursor: 'pointer' }}
                    ></i>
                    <i
                      className="icon_color bi bi-trash ml-2 pt-4"
        ...
        {selectedListLink.length === 0 ? (
          ...
          <div className="form-group py-3 w-100 d-flex justify-content-center gap-3">
            <button
              ...
              className="btn btn-warning shadow my-3"
            >
              Add New Row
            </button>
          </div>
